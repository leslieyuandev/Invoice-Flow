...
import type { MapsJobSummary } from "@/types/maps";

const STATUS_META: Record<
  MapsJobSummary["status"],
...
export function JobProgress({ job, onDelete }: JobProgressProps) {
  ...
  return (
    ...
    >
      ...
      {job.newCount > 0 && <Stat label="🆕 New leads" value={String(job.newCount)} />}
      <Stat label="Queries" value={String(job.searchQueries.length)} />
      <Stat label="Max / term" value={String(job.maxResults)} />
      <Stat label="Language" value={job.language.toUpperCase()} />

      <div className="flex-1 min-w-[8rem] truncate text-sm text-surface-500">
        {job.searchQueries.join(", ")}
      </div>

      {active && (
        <span className="text-xs text-surface-400">
          Scraping… results appear below as they’re found.
        </span>
      )}
