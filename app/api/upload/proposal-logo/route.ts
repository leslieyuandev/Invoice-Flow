...
export async function POST(req: Request) {
  try {
    ...
    const blob = await put(`proposal-logos/${session.user.id}-${Date.now()}`, file, { access: "public" });
    await db.user.update({
      where: { id: session.user.id },
      data: { proposalLogoUrl: blob.url },
    });

    return NextResponse.json({ url: blob.url });
  } catch (err) {
    console.error("Proposal logo upload failed:", err);
